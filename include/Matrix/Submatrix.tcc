...
namespace axi
{
    ...
    template<typename M>
    Submatrix<M>::Submatrix(M& mat, const Index<Range>& row_index, const Index<Range>& col_index) :
    ...
    template<typename M>
    Submatrix<M>::Submatrix(M& mat, const Index<Selected> &selected_rows, const Index<Selected> &selected_cols) :
    ...
    template<typename M>
    void
    Submatrix<M>::fill(value_type value)
    {
        for(auto& x : *this)
        {
            x = value;
        ...
        std::visit(Submatrix_fill{mat_, value}, indexes_);
    }
    ...
    Submatrix<M>::fill(const Index<Selected> &rows, const Index<Selected> &cols, const M& mat)
    {
        ...
        {
            if(are_same_indexes(source_row, rows))
            {
                for(std::size_t source_col = 0, dest_col = 0; source_col < mat.cols(); ++source_col)
                {
                    if(are_same_indexes(source_col, cols))
                    {
